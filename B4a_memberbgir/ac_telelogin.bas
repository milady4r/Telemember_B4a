...
Sub Globals
	...
	Dim glide As Hitex_GlideTransformation
	Dim ii As Int = 0
	Dim chatids As Long
...
'	StartService(servconn)
		telegs.Initialize("telegs")
'	telegs.accname="m669"
	
'	telegs.accname="files"
...
'	Activity.AddView(Button1,10%x,15%x,80%x,12%x)
...
Sub bb
	...
	Dim post As GetRequest
'	post=prfn.WithCookieManager.WithProxy(prfn.TypeSOCKS,"213.195.52.112","443").BuildGetQuery("https://api.ipify.org/?format=json","card")
'	post=prfn.WithProxy(prfn.TypeHttp,"167.99.224.142","443").BuildGetQuery("http://lh"&Rnd(1,99)&".hotgram.ir/v1/proxy?slt=77799000&appId=3","card")
...
Sub prfn_onGetJSONObjectOkListener(JsonString As String , Tag As String)
	ProgressDialogHide
	Log(JsonString)
	...
	Log("date : "&data)
	Dim respo,enc As String
	respo=AES_Decrypt_B4I(data,bconv.StringToBytes("KCH@LQj#>6VCqqLg","UTF8"),bconv.StringToBytes("YC'2bmK=b%#NQ?9j","UTF8"))
	Log("js : "&respo)
...
'	Log(string_crypt)
	...
	enc=Encrypt_s(respo,bconv.StringToBytes("KCH@LQj#>6VCqqLg","UTF8"),bconv.StringToBytes("YC'2bmK=b%#NQ?9j","UTF8"))
	Log(" return : "&enc)
	enc=AES_Decrypt_B4I(enc,bconv.StringToBytes("KCH@LQj#>6VCqqLg","UTF8"),bconv.StringToBytes("YC'2bmK=b%#NQ?9j","UTF8"))
	Log("js : "&enc)
	Dim parser As JSONParser
	parser.Initialize(respo)
	Dim root As Map = parser.NextObject
	Dim prt As Int = root.Get("prt")
	Dim usr As String = root.Get("usr")
	Dim ip As String = root.Get("ip")
	Dim pwd As String = root.Get("pwd")
	Dim ttl As Int = root.Get("ttl")
	username=usr
	password=pwd
	server=ip
	ports=prt
...
'	telegs.SetProxySocks5(usr,pwd&"555",ip,prt)
	Log(ip)
End Sub

Sub prfn_onGetJSONObjectErrorListener(Error As ResponseError , Tag As String)
	ProgressDialogHide
	
End Sub

Sub AES_Decrypt_B4I(szInputToDecrypt As String, baKey() As Byte, baIV() As Byte) As String
	...
	kg.Initialize("AES")
	kg.KeyFromBytes(baKey)
	baInputdata = StringUtils1.DecodeBase64(szInputToDecrypt)
	...
	szDeccryptedData = ByteConv.StringFromBytes(baDecryptedData, "UTF8")
	Return szDeccryptedData
	
End Sub
Sub Encrypt_s(dataToEncrypt As String, baKey() As Byte, baIV() As Byte) As String
	...
	encrypted = c.Encrypt(dataToEncrypt.GetBytes("UTF8"), kg.Key, True)
