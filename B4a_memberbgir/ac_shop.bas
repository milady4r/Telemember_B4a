...
'	cursor1.Close
	...
	lbl_a.Text="فروشگاه سکه"
	lbl_a.Gravity=Gravity.CENTER_VERTICAL+Gravity.CENTER_HORIZONTAL
	pnl_lv.AddView(lbl_a,5%x,0,pnl_lv.Width-10%x,12%x)
...
'	telegs.getChatList(10)
    ...
    Dim pnl_shop As Panel
	pnl_shop.Initialize("pnl_shop")
	pnl_lv.AddView(pnl_shop,0,lbl_a.Top+lbl_a.Height+2%x,pnl_lv.Width,pnl_lv.Height-lbl_a.Top-lbl_a.Height-3%x)
    
	Select typesef
		Case 0:
			Dim mishop As MI_shop
			mishop.Initialize("mishop",Me)
			mishop.type_history=0
			mishop.addpnl(pnl_shop)
			req_igetme(i_pk)
		Case 1:
			Dim mishop As MI_shop
			mishop.Initialize("mishop",Me)
			mishop.type_history=1
			mishop.addpnl(pnl_shop)
			req_getme(userid_t)
...
Sub mishop_newcointele(value As String)
	Log("coin : "&value)
	Dim n_coins=value As Long
	If n_coins>100 Then
